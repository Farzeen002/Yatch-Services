  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const formattedBookings: Booking[] = data?.map((b: any) => ({
        ...
        status: b.status,
        paymentStatus: "paid", // You can adjust this based on your schema
        startDate: b.start_date,
        endDate: b.end_date,
        guests: b.guests,
        location: b.yacht?.location || "",
        price: b.total_price,
        paidAmount: b.total_price, // Adjust if partial
        bookingRef: b.id.slice(0, 8).toUpperCase(),
        customerName: "Customer", // Fetch user info if needed
        ...
        crew: 4,
        media: [
          ...(b.yacht?.images?.map((img: string) => ({ type: "image", src: img })) || []),
          ...(b.yacht?.videos?.map((vid: string) => ({ type: "video", src: vid })) || [])
        ]
      })) || [];
